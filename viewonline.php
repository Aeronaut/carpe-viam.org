...
while ($row = $db->sql_fetchrow($result))
{
	...
	{
		case 'index':
			if (strpos($row['session_page'], 'section=forum') !== false) { $location = $user->lang['INDEX'];  $location_url = append_sid($row['session_page']); }
			elseif (strpos($row['session_page'], 'section=galery') !== false) { $location = $user->lang['GALERY'];  $location_url = append_sid($row['session_page']); }
			else  { $location = $user->lang['WEBSITE'];  $location_url = append_sid($row['session_page']); }
		break;
